...
import { getCustomers, getProduct } from "../_data";
...
import { deleteProducts, updateProduct } from "../_actions";
...
export function UpdateProductForm({ product }: UpdateProductFormProps) {
  ...
  const productImage = product.image ? supabase.storage.from("media").getPublicUrl(product.image).data.publicUrl : null;

  const [customers, setCustomers] = useState<Customer[]>([]);
  const [imageFile, setImageFile] = useState<File | null>(null);
  const [imagePreview, setImagePreview] = useState<string | null>(productImage);
  ...
  async function onDelete() {
    try {
      setIsDeleting(true);
      const [deletedProduct] = await deleteProducts({ orgId, productIds: [product.id] });
      toast.success("Product Deleted", { description: deletedProduct.name });
      router.push(`/orgs/${orgId}/products`);
    } catch (error) {
      console.error(error);
