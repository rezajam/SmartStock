...
import { deleteCustomers, updateCustomer } from "../_actions";
...
export function UpdateCustomerForm({ customer }: { customer: Customer }) {
  ...
  const updatedAt = useMemo(() => {
    return new Date(customer.updated_at).toLocaleDateString("en-US", {
      year: "numeric",
      month: "long",
      day: "numeric",
      hour: "numeric",
  ...
  async function onDelete() {
    try {
      setIsDeleting(true);
      const [deletedCustomer] = await deleteCustomers({ orgId, customerIds: [customer.id] });
      toast.success("Customer Deleted", { description: deletedCustomer.name });
      router.push(`/orgs/${orgId}/customers`);
    } catch (error) {
      console.error(error);
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-start gap-2">
              ...
              <div className="flex flex-col">
                <h1 className="text-2xl font-medium tracking-tight">{customer.name}</h1>
